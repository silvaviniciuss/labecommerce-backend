...
INSERT INTO
    purchases (
        id,
        buyer,
        total_price,creat_at)
VALUES 
    ('c001','u001',499.9, DATETIME('now', 'localtime')),
    ('c002','u002',55.78, DATETIME('now', 'localtime')),
    ('c003','u003',159.99, DATETIME('now', 'localtime'));
...
INNER JOIN purchases AS purchasesBuyer
ON purchases_products.purchase_id = purchasesBuyer.id
INNER JOIN products AS productName
ON purchases_products.product_id = productName.id
INNER JOIN users AS userName
ON purchasesBuyer.buyer = userName.id;
